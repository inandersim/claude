...
| Modül                 | Domain                                                                                    | Repository              | Notlar                                                                                                                                                                       |
| --------------------- | ----------------------------------------------------------------------------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Destinasyonlar**    | `lakeLouiseScore`, `amsAdvice`, `acclimatizationPlan`, `ascentRateWarning`                | `DestinationRepository` | Nepal/Himalaya, Kilimanjaro, Patagonya, Alpler, Türkiye arşivi; "nasıl gidilir / nerede kamp kurulur"; dönüş sözü ve AMS (irtifa hastalığı) takibi (`docs/DESTINATIONS.md`). |
| **Görüntü analizi**   | `situationPrompt`, `parseVisionResponse`, `localVisionAdvice`                             | `VisionRepository`      | Kamera → gateway `/v1/vision` (Claude görüntü); çevrimdışı kural tabanlı yedek.                                                                                              |
| **Ülkeye göre SOS**   | `emergencyNumber`, `nearestCenters`, `sosMessage`                                         | `EmergencyRepository`   | Konumdan ülke tespiti; 40+ ülke kurtarma dizini; cihaz arama.                                                                                                                |
| **Sosyal**            | `parseHashtags`, `parseMentions`, `applyFeedFilter`, `trendingHashtags`                   | `SocialRepository`      | Durum/fotoğraf gönderileri, tepkiler, bahsetme, hashtag, kaydetme, repost, popüler.                                                                                          |
| **Gruplar**           | `canManageGroup`, `groupMembershipOf`, `applyVote`, `pollPercentages`                     | `GroupRepository`       | Telegram tarzı gruplar/kanallar, davet bağlantısı, sabitlenmiş mesaj, medya paylaşımı; polling ile canlı.                                                                    |
| **Eğitimler**         | `progressOf`, `nextLesson`, `canAccessLesson`, `isCertificateValid`                       | `CourseRepository`      | Kurs kataloğu (WFA/WFR, AIARE, PADI, tırmanış, navigasyon…), dersler, sınav, sertifika, canlı oturumlar.                                                                     |
| **Topluluk rotaları** | `trackStats`, `simplifyTrack`, `bearing`, `destinationPoint`                              | `TrackRepository`       | Kullanıcı GPS/medya kayıtlarından sanal yollar ve kamp POI'leri; GPX/Strava içe aktarma; adım adım navigasyon (`docs/COMMUNITY_TRAILS.md`).                                  |
| **Hava & çığ**        | `computeAlerts`, `windChill`, `freezingLevel`, `bestWindow`                               | `WeatherRepository`     | Open-Meteo (hava, yükseklik) + EAWS çığ bülteni; önbellekli, çevrimdışı yedekli (`docs/OPEN_DATA.md`).                                                                       |
| **Ülke rehberi**      | `visaLabel`, `visaRequiresApplication`, `filterCountries`, `sortByRelevance`              | `CountryRepository`     | 42 ülke: sosyal görgü, güvenlik uyarıları, yasalar, vize ve evrak yol haritası, kontrol listesi.                                                                             |
| **Yazarlar & blog**   | `readMinutes`, `parseArticleBody`, `uniqueSlug`, `excerpt`                                | `ArticleRepository`     | Yazar/blogger sayfaları, makaleler, takip, koleksiyonlar.                                                                                                                    |
| **Canlılar**          | `filterSpecies`, `dangerMeta`, `isDeterrentAnimal`, `isDeterrentSound`                    | `WildlifeRepository`    | 40 tür veritabanı, fotoğraftan tanıma (`/v1/species`), topluluk soru-cevap, kaçırma sesi kütüphanesi (`assets/sounds`), acil canlı tepkisi düğmesi.                          |
| **Tele-tıp**          | `localTriage`, `matchDoctors`, `urgencyMeta`, `isConsultOpen`                             | `TelemedRepository`     | Çevrimiçi doktor kuyruğu, yılan/böcek/hayvan ısırığı protokolleri, gateway `/v1/triage`.                                                                                     |
| **Zirtan TV**         | `scheduleForDay`, `nowPlaying`, `nextEpisode`, `newsSeverityMeta`                         | `TvRepository`          | Belgesel/haber/canlı kanallar, yayın akışı, haber şeridi, sonra izle, video gönderme.                                                                                        |
| **Tarihi alanlar**    | `heritageVisitPlan`, `heritageOpenNow`, `sortGuideStops`, `heritageTourSummary`           | `HeritageRepository`    | 31 arkeolojik/tarihi alan, sesli rehber durakları, en yakın komşu ziyaret planı, turlar, ziyaret XP'si; rota ve destinasyon çapraz bağlantısı.                               |
| **Çocuk**             | `kidAgeBandMeta`, `kidPlaceOpenInMonth`, `withKidPlaceDistance`, `KID_STICKER_THRESHOLDS` | `KidsRepository`        | 28 çocuk dostu yer, çocuk profilleri, Küçük Kâşif doğa avı ve çıkartmalar, aile kontrol listesi.                                                                             |

**Modül i18n:** Her modülün çevirileri `core/i18n/modules/<modül>.ts` içinde (`tr` kaynak, `en` zorunlu, `XI18nShape` tipi) ve diğer 22 dil `core/i18n/modules/locales/<dil>/<modül>.ts` dosyalarında; `localeSet()` eksik dili İngilizceye düşürür. `tr.ts`/`en.ts` yalnızca `ai: aiI18n.tr` gibi bağlar.
...
`core/theme/tokens.ts` açık (doğa/kum), koyu (sıcak orman) ve güneş (yüksek kontrast) paletlerini, tipografi ölçeğini (Manrope) ve boşluk/yarıçap ölçeğini tanımlar. `ThemeProvider` kullanıcı tercihini (otomatik/sistem/açık/koyu/güneş) kalıcı tutar; otomatik modda Android ışık sensörü (`core/theme/ambient.ts`, histerezisli lux eşiği), diğer platformlarda güneş doğuş/batış saatine göre şema seçilir. Bileşenler renkleri yalnızca `useTheme().colors` üzerinden alır.

## Uluslararasılaştırma

`core/i18n` altında `tr.ts` kaynak sözlük; 22 diğer dil (`languages.ts` içindeki `LANGUAGE_META`: en, de, fr, es, it, ja, pt, ru, zh, ko, hi, ne, ar, ka, el, pl, cs, nl, sv, nb, id, th) aynı tipe uymak zorundadır (`Translations`). Arapça için `applyDirection` RTL yönünü web'de `dir`, yerelde `I18nManager` ile uygular. `t('home.altitude')` anahtarları derleme zamanında kontrol edilir; dil değişimi Zustand store üzerinden tüm ekranları yeniden çizer.
